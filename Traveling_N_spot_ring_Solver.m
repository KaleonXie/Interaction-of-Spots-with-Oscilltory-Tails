...
Stationary_N_spot_ring_Solver; % Get Stationary N-spot ring first; If you have already got the stationary steady state, ignore this line.
...
Om_v=Om_v-0.005*(dx_u)+0.005.*(dy_u); % add a uniform speed to the stationary state.
...
%       end
    ...
    title ([' t = ',num2str(t,'%4.3f'), ' $\tau$ = ',num2str(tau,'%4.3f')], 'interpreter', 'latex', 'fontsize', 12);
    colorbar;
    grid on
    grid minor
	shading interp;
    ...
    axis equal
    hold off
    imag_name=sprintf("Traveling%dSpot%d.png",spots_num,imag_index)
...
AjustTrajectoriesm; %
save('Traveling_trajecotories.mat','ts','newxc','newyc')
figure(3)
